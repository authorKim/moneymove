...
export default function Dividends() {
  const { dividends, addDividend, updateDividend, deleteDividend } = useSupabaseStore()
  const { user } = useAuth()
  ...
  const handleAmountChange = (val: string) => {
    ...
    if (!isNaN(num)) update('amount', num)
    else if (val === '' || val === '-') update('amount', 0)
  }

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (editItem) {
      updateDividend({ ...form, id: editItem.id }, uid)
      setEditItem(null)
    } else {
      addDividend({ ...form, id: genId() }, uid)
    }
    setForm({ year: new Date().getFullYear(), month: new Date().getMonth() + 1, stockName: '', amount: 0 })
    setRawAmount('')
  }
  ...
  const monthlyByYear = years.map((year) => {
    const monthly = Array.from({ length: 12 }, (_, i) => {
      const m = i + 1
      const total = dividends.filter((d) => d.year === year && d.month === m).reduce((s, d) => s + d.amount, 0)
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <h2 className="text-xl md:text-2xl font-bold text-gray-900">배당 내역</h2>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        {/* Input Form */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 md:p-5">
          <h3 className="font-semibold text-gray-900 mb-4">{editItem ? '배당 수정' : '배당 입력'}</h3>
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div className="flex gap-2">
              ...
              {editItem && (
                <button type="button" onClick={cancelEdit} className="flex-1 bg-gray-100 text-gray-700 rounded-lg py-2 text-sm font-medium hover:bg-gray-200">
                  취소
                </button>
              )}
            </div>
        ...
        <div className="col-span-1 md:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <div className="hidden md:block">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {sorted.length === 0 && (
                  <tr><td colSpan={5} className="text-center py-8 text-gray-400">배당 내역을 입력해주세요</td></tr>
                )}
                {sorted.map((d) => (
                  <tr key={d.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-right">
                      <button onClick={() => startEdit(d)} className="text-blue-600 hover:underline text-xs mr-2">수정</button>
                      <button onClick={() => { if (confirm('삭제하시겠습니까?')) deleteDividend(d.id) }} className="text-red-500 hover:underline text-xs">삭제</button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="md:hidden divide-y divide-gray-100">
            {sorted.length === 0 && (
              <div className="text-center py-8 text-gray-400 text-sm">배당 내역을 입력해주세요</div>
            )}
            {sorted.map((d) => (
              <div key={d.id} className="p-4 flex items-center justify-between">
                <div>
                  <div className="font-medium text-gray-900">{d.stockName}</div>
                  <div className="text-xs text-gray-500">{d.year}년 {d.month}월</div>
                </div>
                <div className="text-right">
                  <div className="text-sm font-mono text-green-600 font-semibold">+{d.amount.toLocaleString()}원</div>
                  <div className="flex gap-2 mt-1">
                    <button onClick={() => startEdit(d)} className="text-xs text-blue-600">수정</button>
                    <button onClick={() => { if (confirm('삭제?')) deleteDividend(d.id) }} className="text-xs text-red-500">삭제</button>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="space-y-4">
          {monthlyByYear.map(({ year, monthly, annual }) => (
            <div key={year} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="px-4 md:px-6 py-3 border-b border-gray-100 flex justify-between items-center">
                <h3 className="font-semibold text-gray-900">{year}년 배당 요약</h3>
                <span className="text-sm font-mono text-green-600 font-semibold">
                  연간: +{formatKRW(annual)}
                </span>
              </div>
              <div className="hidden md:grid grid-cols-12 divide-x divide-gray-100 text-center">
              ...
              <div className="md:hidden px-4 py-3">
                <div className="grid grid-cols-4 gap-2">
                  {monthly.filter(m => m.total > 0).map(({ month, total }) => (
                    <div key={month} className="text-center bg-green-50 rounded-lg py-2">
                      <div className="text-xs text-gray-500">{month}월</div>
                      <div className="text-xs font-mono text-green-600 font-semibold">{total.toLocaleString()}</div>
                    </div>
                  ))}
                  {monthly.filter(m => m.total > 0).length === 0 && (
                    <div className="col-span-4 text-center text-gray-400 text-xs py-2">배당 없음</div>
                  )}
                </div>
              </div>
            </div>
